...
public class VenteVin {
	...
	public boolean mettreEnAttente(String client) {
		...
		int nbBouteillesCommandees = mapClientCommande.get(client).getNombreBouteillesDemandees();
		tableFilesDAttente[nbBouteillesCommandees].add(client);
	...
	public String selectionnerClientSuivant(){
		for (ArrayDeque<String> fileDAttente : tableFilesDAttente) {
			if(!fileDAttente.isEmpty()) {
				String client = fileDAttente.remove();
	...
	public boolean demanderUneBouteille(String client){
		if (client == null || client.isBlank()) throw new IllegalArgumentException();
		if(nombreBouteillesRestantes==0) return false;
