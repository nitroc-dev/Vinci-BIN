...
public class ListeBalPromo {
	...
	public boolean ontTous16AnsOuPlus(){
		...
		Noeud baladeur = tete;
		return baladeur.lyceen.getAge() >= 16 && ontTous16AnsOuPlus(baladeur.suivant);
	}

	private boolean ontTous16AnsOuPlus(Noeud baladeur){
		if (baladeur == null) return true;
		return baladeur.lyceen.getAge() >= 16 && ontTous16AnsOuPlus(baladeur.suivant);
	}
	...
	public Lyceen donnerPartenaire(Lyceen lyceen){
		...
		Noeud noeud = mapLyceenNoeud.get(lyceen);
		if(noeud==null) return null;
		if (lyceen.getSexe() == 'G') {
			return noeud.suivant.lyceen;
		} else {
			return noeud.precedent.lyceen;
		}
	}
	...
	public boolean ajouterCouple (Lyceen lyceen1, Lyceen lyceen2) {
		...
		Noeud nouveauNoeud1;
		Noeud nouveauNoeud2;
		if (lyceen1.getSexe() == 'G') {
			nouveauNoeud1 = new Noeud(lyceen1);
			nouveauNoeud2 = new Noeud(lyceen2);
		} else {
			nouveauNoeud1 = new Noeud(lyceen2);
			nouveauNoeud2 = new Noeud(lyceen1);
		}
		if (mapLyceenNoeud.containsKey(lyceen1) || mapLyceenNoeud.containsKey(lyceen2)) return false;
		if (lyceen1.getSexe() == lyceen2.getSexe()) return false;
		if (estVide()) {
			tete = nouveauNoeud1;
			queue = nouveauNoeud2;
			tete.suivant = queue;
		...
		mapLyceenNoeud.put(lyceen1, nouveauNoeud1);
		mapLyceenNoeud.put(lyceen2, nouveauNoeud2);
		return true;
	}
