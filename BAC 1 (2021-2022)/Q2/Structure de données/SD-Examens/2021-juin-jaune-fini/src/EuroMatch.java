...
public class EuroMatch {
	...
	public boolean placerDansFileAttente(String client){
		...
		if (nombrePlacesRestantes == 0) return false;
	...
	public boolean passerNouvelleReservation(String client, int nombrePlacesDemandees){
		...
		if (nombrePlacesDemandees > MAX_PLACES_CLIENT) throw new IllegalArgumentException();
		if (nombrePlacesDemandees > nombrePlacesRestantes) return false;
	...
	public boolean modifierReservation(String client,int nombrePlacesDemandeesEnPlus){
		if (client == null || client.isEmpty()) throw new IllegalArgumentException();
		if (nombrePlacesDemandeesEnPlus <= 0) throw new IllegalArgumentException();
		if (!mapClientReservation.containsKey(client)) throw new IllegalStateException();
		Reservation reservation = mapClientReservation.get(client);
		int nombrePlacesRestantesApresModification = reservation.getNombrePlacesReservees() + nombrePlacesDemandeesEnPlus;
		if (nombrePlacesRestantesApresModification > MAX_PLACES_CLIENT) throw new IllegalArgumentException();
		if (nombrePlacesRestantesApresModification > nombrePlacesRestantes) return false;
		reservation.setNombrePlacesReservees(nombrePlacesRestantesApresModification);
