...
public class GestionFancyFair {
	...
	public static void main(String[] args) {	
		...
		scanner.nextLine();
		fancyFair = new FancyFair(nombreTotalPlaces, nombrePlacesEnfants);
		System.out.println();
	...
	private static void reserverEnfant() {
		...
		ArrayList<Integer>	liste = fancyFair.reserverEnfant(nom, nombrePlaces);
		if(liste==null){
	...
	private static void reserverAdulte() {
		...
		scanner.nextLine();
		if(fancyFair.reserverAdulte(nom, liste)){
	...
	private static void reserverAutomatiquement() {
		...
		System.out.print("Entrez le nom : ");
		String nom = scanner.nextLine();
		int numero = fancyFair.attribuerAutomatiquementPlace(nom);	
		if(numero==-1){
			System.out.println("La reservation a echoue!");
		}else{
