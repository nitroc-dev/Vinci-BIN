...
public class Camion  {
    ...
    public List<Trajet> trouverTrajetsAvecPlaceRestante(){
        return trajets.stream().filter(caisses -> caisses.nbCaisses() < nbMaxCaisses).sorted(Comparator.comparing(Trajet::getVilleDepart)).sorted(Comparator.comparing(Trajet::getVilleArrivee)).collect(Collectors.toList());
    }
    ...
    public LocalDate trouverDateTrajet(String villeDepart, String villeDArrivee){
        return trajets.stream().filter(caisses -> caisses.getVilleDepart().equals(villeDepart) && caisses.getVilleArrivee().equals(villeDArrivee)).map(Trajet::getDate).findFirst().isPresent() ? trajets.stream().filter(caisses -> caisses.getVilleDepart().equals(villeDepart) && caisses.getVilleArrivee().equals(villeDArrivee)).map(Trajet::getDate).findFirst().get() : null;
    }
