...
public class Camion  {
    ...
    public boolean ajouterTrajet(Trajet trajet){
        ...
        for (LocalDate date : trajets.keySet()) {
            ...
            if (date.plusDays(1).equals(trajet.getDate()) && !trajets.get(date).getVilleArrivee().equals(trajet.getVilleDepart())) return false;
            if (date.minusDays(1).equals(trajet.getDate()) && !trajets.get(date).getVilleDepart().equals(trajet.getVilleArrivee())) return false;
        }
    ...
    public Trajet trouverTrajet(LocalDate dateTrajet){
        for (LocalDate date : trajets.keySet()) {
            if (date.equals(dateTrajet)) return trajets.get(date);
        }
        return null;
    }
