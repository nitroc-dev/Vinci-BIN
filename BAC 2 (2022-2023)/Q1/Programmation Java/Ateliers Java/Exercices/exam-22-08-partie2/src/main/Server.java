...
public class Server {
    ...
    public void listenKeyboard() {
        ...
        while (true) {
            ...
            // Create Request
            Request request = RequestFactory.createRequest(split[0], split[1]);
            // Delegate request handling
            HandleRequestThread thread = new HandleRequestThread(request);
            thread.start();
        }
    }
    ...
    private Method getMethod(Request request) {
        for (Class clazz : getResourcesClasses()) {
            for (Method method : clazz.getDeclaredMethods()) {
                if (method.isAnnotationPresent(Path.class) && Modifier.isPublic(method.getModifiers()) && !Modifier.isStatic(method.getModifiers())) {
                    Path path = method.getAnnotation(Path.class);
                    if (path.value().equalsIgnoreCase(request.getURL())) {
                        if (request.getMethod().equalsIgnoreCase("GET") && method.isAnnotationPresent(GET.class)) {
                            return method;
                        } else if (request.getMethod().equalsIgnoreCase("POST") && method.isAnnotationPresent(POST.class)) {
                            return method;
                        }
                    }
                }
    ...
    private class HandleRequestThread extends Thread {
        ...
        public HandleRequestThread(Request request) {
